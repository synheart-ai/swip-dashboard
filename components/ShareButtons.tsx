...
export function ShareButtons({ type, appName, rank, score, leaderboardData }: ShareButtonsProps) {
  ...
  const getShareText = () => {
    if (type === 'app' && appName && rank && score) {
      return `🏆 ${appName} ranked #${rank} on the SWIP Global Leaderboard with a wellness score of ${score.toFixed(1)}! Check it out: ${window.location.origin}/leaderboard`;
    }
  ...
  const shareToLinkedIn = () => {
    const url = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.origin + '/leaderboard')}`;
  ...
  const exportAsCSV = () => {
    ...
    const headers = ['Rank', 'App Name', 'Category', 'SWIP Score', 'Stress Rate', 'Sessions'];
    ...
    link.href = url;
    link.download = `swip-leaderboard-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  ...
  const exportAsJSON = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `swip-leaderboard-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
    URL.revokeObjectURL(url);
    setShowExportMenu(false);
  };
  ...
  const captureScreenshot = async () => {
    ...
    try {
      ...
      const element = document.querySelector('[data-screenshot-container]') as HTMLElement;
      if (!element) {
      ...
      const link = document.createElement('a');
      link.download = `swip-leaderboard-${new Date().toISOString().split('T')[0]}.png`;
